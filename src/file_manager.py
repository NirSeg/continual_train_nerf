...
class FileManager:
    ...
    def __init__(self, path_to_config: str | os.PathLike | Path):
        ...
        self._project_name = self._config_path.relative_to('/workspace/outputs/').parents[-2].name
        self._data_name = self._config_path.relative_to('/workspace/outputs/').parents[-3].name
        self._num_set = int(self._data_name[-1])
        self.BASE_DATA_PATH = self.BASE_DATA_PATH.joinpath(self._project_name)
        self._path_to_data = self.BASE_DATA_PATH.joinpath(self._data_name)

        self._train_name = self._config_path.parent.name
        self._path_to_output = self.BASE_OUTPUTS.joinpath(
            f'{self._project_name}/{self._data_name}/nerfacto/{self._train_name}')
        ...
        self._renders_path = self.BASE_RENDERS.joinpath(f'{self._project_name}/{self._data_name}')
    ...
    def viewer_next_poses(self, all_poses: bool = True, update_poses: bool = False):
        if self._dp_outputs is None:
            update_poses = True

        next_transforms_path = os.path.dirname(os.path.dirname(self._transforms_path))
        next_transforms_path = os.path.join(next_transforms_path,
                                            f'set_{self._num_set + 1}/{os.path.basename(self._transforms_path)}')
        temp_transform_path = self._transforms_path
        self._transforms_path = next_transforms_path

        path_to_next_data = Path(os.path.join(os.path.dirname(self._path_to_data), f'set_{self._num_set + 1}'))
        temp_path_to_data = self._path_to_data
        self._path_to_data = path_to_next_data
        ...
        self._transforms_path = temp_transform_path
        self._path_to_data = temp_path_to_data
        return self._viewer_poses
    ...
    def generate_cam_path_file(self, filename: str, fov: int, fps: float,
        ...
        image.close()
        start_keyframe = Keyframe(self.viewer_poses()[0].T, fov)
        end_keyframe = Keyframe(self.viewer_poses()[len(self.viewer_poses()) - 1].T, fov)
